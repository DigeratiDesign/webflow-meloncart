(() => {
    ...
    /*
     * MC Motion Core should normally create this API before
     * the debugger loads. This fallback keeps the debugger
     * usable on pages where the core has accidentally been
     * omitted.
     */
  ...
  `;
    ...
    function read(instance, schema, key) {
    ...
    function write(instance, schema, key, value) {
    ...
    function createRange(instance, schema, control) {
    ...
    function createButton(instance, schema, control) {
    ...
    function createStats(schema) {
        ...
        schema.stats.forEach((stat) => {
            ...
            value.textContent = typeof stat.format === 'function'
                ? stat.format(raw)
    ...
    function createSection(instance, schema, index, total) {
        ...
        (schema.controls || []).forEach((control) => {
            ...
            if (control.type === 'range') {
                ...
                element = createButton(instance, schema, control);
            }
    ...
    function renderSchema(content, schema) {
        const instances = typeof schema.instances === 'function'
            ? (schema.instances() || []).filter(Boolean)
            : [];
    ...
    const queued = Array.isArray(window.MC.__debugQueue)
        ? window.MC.__debugQueue.splice(0)
        : [];
