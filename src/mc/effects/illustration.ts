...
type IllustrationSequenceController = {
  element: HTMLElement;
  illustrations: HTMLElement[];
  settings: SequenceSettings;
  get: (key: string) => unknown;
  set: (key: string, rawValue: unknown) => void;
  rebuild: () => void;
  showFinal: () => void;
  replay: () => void;
  destroy: () => void;
...
type MCIllustrationNamespace = MCNamespace & {
  illustrationSequences?: IllustrationSequenceController[];
};

const ensureMC = (): MCIllustrationNamespace => {
  window.MC ||= {};

  return window.MC as MCIllustrationNamespace;
};
...
const getSequenceConfig = (section: HTMLElement): SequenceSettings => {
...
const getSvgLength = (element: Element | null) => {
...
const prepareIllustration = (element: HTMLElement) => {
  ...
  switch (type) {
    case 'storefront': {
      ...
      const centrePieces = element.querySelectorAll<HTMLElement>('.centre');
      const wings = element.querySelectorAll<HTMLElement>('.wing');
...
const createStorefrontAnimation = (element: HTMLElement, duration: number) => {
  const centreAwning = element.querySelector('.awning.a2');
  const outerAwnings = element.querySelectorAll('.awning.a1, .awning.a3');
...
const createLinkedFormsAnimation = (element: HTMLElement, duration: number) => {
...
const createLinkedMechanismsAnimation = (element: HTMLElement, duration: number) => {
  const mechanisms = element.querySelectorAll('.mechanism');
...
const createFoundationCoreAnimation = (element: HTMLElement, duration: number) => {
...
const createFoundationBuildAnimation = (element: HTMLElement, duration: number) => {
...
const createFoundationFreedomAnimation = (element: HTMLElement, duration: number) => {
...
const createFoundationOwnershipAnimation = (element: HTMLElement, duration: number) => {
...
const createIllustrationTimeline = (element: HTMLElement, duration: number) => {
  const type = element.getAttribute('mc-illustration');
...
const showIllustrationFinal = (element: HTMLElement, duration: number) => {
  ...
  const timeline = createIllustrationTimeline(element, duration);
...
const sequenceControllers: IllustrationSequenceController[] = [];

const createSequenceController = (section: HTMLElement, sectionIndex: number) => {
...
const initIllustrationSequences = () => {
  const sections = document.querySelectorAll<HTMLElement>('[mc-illustration-sequence]');

  sequenceControllers.splice(0, sequenceControllers.length);
